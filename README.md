...
It looks at real time straming responses and renders Markdown and Mermaid diagrams.

## Usage

Register for an account and get an API key with OpenAI, TogetherAI, Groq or LeptonAI.  
Write your key it in `.env` file.
...
open FsChat
```
Choose a GPT model and start a chat session:
```fsharp
...
then you will get live Markdown text previews:
...
You can also ask it to render the above table into a chart:
```fsharp
...
The result of each `send` method is a `Response` record with:
...
The above example contains some fancy HTML/CSS formatting. Look at example [dialog.ipynb](docs/dialog.ipynb) for more details and read about how to customize live output rendering below.
