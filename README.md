...
```
When you `chat.send` a prompt:
1) prompt messages that you provided in `send` are first added to context,
2) then LLM is called with all these context messages,
3) and finally response from LLM is also added to context.

Context can be accessed using the `chat.messages` property.  
It returns a C# List / F# ResizeArray of previous interactions like the example above and it can be freely modified in-place (watch out, you're dealing with mutable state).
...
Besides accessing the context using `chat.messages`, you can also store the response of a call to `chat.send` into a variable and access it.

The result of `chat.send` is a `Response` record with:
